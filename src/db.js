...
export class StatsStore {
  constructor(db) {
    ...
    this._insertMatch = db.prepare(`
      INSERT INTO match_stats (
        ...
        started_at, ended_at, kills, deaths, cash_end, cash_peak, cash_earned, won
      ) VALUES (
        @serverId, @steamId, @name, @faction, @map, @mode,
        @startedAt, @endedAt, @kills, @deaths, @cashEnd, @cashPeak, @cashEarned, @won
      )
    `);
  ...
  recordMatch(serverId, snapshot, meta) {
    ...
    const row = {
      ...
      cashEarned: snapshot.cashEarned || 0,
  ...
  top(metric, limit = 100, serverId = "") {
    ...
    if (metric === "match_kills" || metric === "match_cash") {
      const valueColumn = metric === "match_cash" ? "cash_earned" : "kills";
      const scopeWhere = scope ? `WHERE m.server_id = '${scope}'` : "";
      const positive = metric === "match_cash" ? "cash_earned > 0" : "kills > 0";
      return this.db.prepare(`
        WITH ranked AS (
          SELECT m.*,
                 ROW_NUMBER() OVER (
                   PARTITION BY m.steam_id
                   ORDER BY m.${valueColumn} DESC, m.kills DESC, m.ended_at DESC
                 ) AS rn
          FROM match_stats m
          ${scopeWhere}
        )
        SELECT steam_id, name, kills, deaths, cash_end, cash_peak, cash_earned,
               server_id, map, mode, started_at, ended_at
        FROM ranked
        WHERE rn = 1 AND ${positive}
        ORDER BY ${valueColumn} DESC, kills DESC, ended_at DESC
        LIMIT ${n}
      `).all();
    }
    const board = scope

...
import { applyTick, emptyState, matchAwards } from "./logic.js";
...
export class Poller {
  ...
  lastAwards(serverId) {
    return this.awards.get(String(serverId)) || null;
  }
  ...
  persist(server, state, events) {
    ...
    for (const event of events) {
      ...
      if (event.type === "match_end") {
        matchEnded = true;
        const awards = matchAwards(event.snapshots);
        this.awards.set(server.id, {
          ...awards,
          map: event.map || "",
          mode: event.mode || "",
          endedAt: now,
